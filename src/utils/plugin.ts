...
export function resolveSettings(settings: MinimalTheoremCalloutSettings, plugin: CrossLinksPlugin, currentFile: TAbstractFile): ResolvedMathSettings;
export function resolveSettings(settings: undefined, plugin: CrossLinksPlugin, currentFile: TAbstractFile): Required<MathContextSettings>;
export function resolveSettings(settings: MinimalTheoremCalloutSettings | undefined, plugin: CrossLinksPlugin, currentFile: TAbstractFile): Required<MathContextSettings> {
...
export function getProfile(plugin: CrossLinksPlugin, file: TFile) {
    const settings = resolveSettings(undefined, plugin, file);
    const profile = plugin.extraSettings.profiles[settings.profile];
    return profile;
}

export function getProfileByID(plugin: CrossLinksPlugin, profileID: string) {
    const profile = plugin.extraSettings.profiles[profileID];
    return profile;
}

export function staticifyEqNumber(plugin: CrossLinksPlugin, file: TFile) {
...
export async function insertBlockIdIfNotExist(plugin: CrossLinksPlugin, targetFile: TFile, cache: CachedMetadata, block: MarkdownBlock, length: number = 6): Promise<{ id: string, lineAdded: number } | undefined> {
...
export async function rewriteTheoremCalloutFromV1ToV2(plugin: CrossLinksPlugin, file: TFile) {
...
export function isTheoremCallout(plugin: CrossLinksPlugin, type: string) {
...
export function insertProof(plugin: CrossLinksPlugin, editor: Editor, context: MarkdownView | MarkdownFileInfo) {
